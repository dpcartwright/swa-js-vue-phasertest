...
export default class Enemy extends MatterEntity {
    ...
    constructor(data) {
        ...
        let dropsTxt = JSON.parse(enemy.properties.find(p => p.name == 'dropstxt').value);
        let health = enemy.properties.find(p => p.name == 'health').value;
        super({ scene, x: enemy.x, y: enemy.y, texture: 'enemies', frame: `${enemy.name}_idle_1`, drops, dropsTxt, health, name: enemy.name });
    ...
    update() {
        if (this.dead) return;
        if (this.agro) this.attacking = this.agro;
        if (this.attacking) {
